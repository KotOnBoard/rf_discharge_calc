...
def PairRevolver(pair, par, iter_par, progress):
    ...
    for index in range(len(pair_par[pair[0]])):
        ...
        out, progress = RecurIterator(par, iter_par, progress)
        if (len(output)==0): output = out
        else: output = pd.concat([output, out], axis=0)
...
def printProgressBar (progress, err, printEnd = "\r"):
    ...
    match err:
        ...
        case 5: progress[5]+=1
        case 6: progress[6]+=1          
    print(f'\rIteration: {progress[0]}|Te: {progress[0]-np.sum(progress[1:2])}/{progress[0]} |Vrf: {progress[0]-np.sum(progress[1:3])}/{progress[0]} |Ubias: {progress[0]-np.sum(progress[1:4])}/{progress[0]} |dEi: {progress[0]-np.sum(progress[1:5])}/{progress[0]} |Selectivity: {progress[0]-np.sum(progress[1:6])}/{progress[0]}', end = printEnd)
    return progress
